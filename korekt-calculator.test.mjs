...
test("пътуваща бригада: дълъг път дава повече от един ден", () => {
  // ръчно зададено разстояние — за да не зависи тестът от roadSpeed/реалната
  // география на София→Созопол (при по-висока скорост "дълъг" вече не е > 1 ден)
  const r = дълъг({ courseMode: "dayCrew", kmOverride: 1200 });
