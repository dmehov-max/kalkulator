...
const NEIGHBORHOODS = {
  ...
  "Пловдив": [
    ...
    { name: "с. Белащица", lat: 42.065, lng: 24.742 },
  ],
  "Варна": [
...
async function fetchRealDistanceKm(cityA, cityB, apiKey) {
  ...
  try {
    ...
    // взимаме препоръчания от Google маршрут (първия) — обикновено през магистрала/
    // главен път, не непременно най-краткия по километри. Реалните камиони карат по
    // магистралата (напр. София—Пловдив по Тракия), а не по най-късия път по вади.
    const best = routes[0];
...
function computePrice(s, p) {
  ...
  if (dayCrewMode) {
    // цената към клиента е на ден/час × хора (не по-долу), но реалните човекочасове
    // все пак трябва да влязат в manHours — иначе "Време труд" показва само опаковането,
    // а себестойността (laborCost = manHours × workerCostRate) излиза почти нулева,
    // въпреки че реално плащаме на екипа за целия ден
    const people = n(p.travelCrew || 1) + 1; // + шофьорът, който също товари/разтоварва на място
    handlingManHours = dayTotalH * people;
...
function sanitizeNumericParams(obj, defaults) {
  const out = { ...obj };
  for (const key of Object.keys(defaults)) {
    const dv = defaults[key], v = out[key];
    if (typeof dv === "number") {
      if (v === "" || v == null || !Number.isFinite(Number(v))) out[key] = dv;
    } else if (dv && typeof dv === "object" && !Array.isArray(dv) && v && typeof v === "object" && !Array.isArray(v)) {
      out[key] = sanitizeNumericParams(v, dv);
    }
  }
  return out;
}
...
// извиква Supabase Edge Function (пази service_role ключа сървърно) — виж
// supabase/functions/admin-users/index.ts. Заявката носи access_token-а на текущата
// сесия, функцията сама проверява сървърно дали викащият е PARAMS_EDITOR_EMAIL.
// Живее в самия Supabase проект (не на Netlify) — работи еднакво от GitHub Pages
// и от Netlify, без да зависи от хостинг на статичния сайт.
const ADMIN_FUNCTION_URL = `${DEFAULTS.supabaseUrl}/functions/v1/admin-users`;
...
export default function KorektCalculator() {
  ...
  // зарежда стар запис обратно в съветника за редакция — пази СЪЩИЯ ключ/номер, за да
  // промените обновят оригинала (PATCH), вместо да трупат нов запис (№2, №3...) при всяка
  // редакция на едно и също нещо
  const loadRecordForEditing = (rec) => {
    ...
    recordKey.current = rec.key || null;
